...
class MapStateTest {
    ...
    @Test
    fun `emits initial mapped value`() = runTest {
    ...
    @Test
    fun `emits when mapped value changes`() = runTest {
        val source = MutableStateFlow(1)
        val mapped = source.mapState { it * 10 }
    ...
    @Test
    fun `does not re-emit when mapped value stays the same`() = runTest {
    ...
    @Test
    fun `does not re-emit null when multiple source values map to null`() = runTest {
        val source = MutableStateFlow<String?>("loading")
        val mapped = source.mapState { value ->
            when (value) {
    ...
    @Test
    fun `emits again after mapped value changes back to previous value`() = runTest {
        val source = MutableStateFlow(1)
        val mapped = source.mapState { it * 10 }
