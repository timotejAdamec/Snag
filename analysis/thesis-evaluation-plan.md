...
- **190 Gradle build modules** decomposed into **310 architectural units** (`(module, source_set)` pairs) across 6 running platform targets (Android, iOS, JVM desktop, JS web, WASM web, JVM backend). The architectural unit, not the Gradle module, is the unit of accounting throughout the evaluation chapter — see §4.1's *Unit of measurement* note for the rationale.
...
**Unit of measurement — *architectural unit* vs *build module*.** The chapter distinguishes two senses of the word "module" and uses each consistently:

- **Architectural unit** = a `(module, source_set)` pair. Primary unit for every measurement in §4.2 (sharing), §4.3 (blast radius), §4.4–§4.7 (ripple). Source set is where platform reach lives, where dependency declarations are made, and where the KMP `dependsOn` hierarchy expresses the architecture. Treating each source set as its own unit removes a structural asymmetry: a backend Gradle module is structurally a single `main` source set on a JVM target — if KMP could host JVM-server alongside Android (it cannot, as `jvm()` and the Android target conflict), the BE code would just be the `jvmMain` source set of the same KMP module. Counting the BE module as one unit but bundling FE source sets into one unit per Gradle module would inflate the FE side relative to the BE side. The architectural-unit framing makes BE peer to a KMP module's `commonMain[FE]`, `androidMain`, `iosMain`, `jvmMain`, `jsMain`, `wasmJsMain` source sets, not peer to the whole KMP module.
- **Build module** = a Gradle project (one `build.gradle.kts`, one plugin set, one set of conventions). Used only when discussing build economy — convention-plugin coverage, Gradle-level rebuild scope, build performance — not for sharing or ripple.

Where a count is reported, the chapter names the unit explicitly (e.g., "26 architectural units across 12 build modules"). The ripple closure (§4.3) gives both `blast_radius_unit` (headline, exact under api/implementation scope plus source-set hierarchy) and `blast_radius_module` (build-economy supporting figure: "how many `build.gradle.kts` rebuild"). The `analysis/source_set_hierarchy.py` static lookup encodes Snag's KMP source-set `dependsOn` graph (`commonMain` → `mobile`/`nonWeb`/`nonAndroid`/`nonJvm` intermediates → leaf source sets) as the formal object underlying the unit-level closure.
...
3. **Potential blast radius per touched unit** — for each touched unit, the size of its downstream dependency closure (how many other architectural units transitively consume it). Computed statically by `analysis/dependency_closure.py`, which combines the inter-module Gradle dependency graph (api/implementation scope-aware — implementation edges terminate forward propagation, directly implementing Snag's Action Version Transparency) with the intra-module KMP source-set `dependsOn` hierarchy encoded in `analysis/source_set_hierarchy.py`. Single-target consumers (BE modules' `main`, Android-app modules' `main`) are propagated via target-binary reachability — a KMP source set's change reaches a BE consumer's `main` iff the source set reaches the JVM target binary. The result is exact under the combined api/impl-scope and source-set-hierarchy semantics; the headline `blast_radius_unit` is no longer an upper bound. The `blast_radius_module` companion answers the build-economy question of how many `build.gradle.kts` files rebuild. It is *not* added to the ripple count but is reported as a per-unit annotation. The intuition: a touch in `commonMain` of `feat/projects/business/model` has a larger blast radius than a touch in `iosMain` of `feat/projects/fe/driven/impl` because more downstream source sets depend on it. Whether that blast radius *actually* materialized as a secondary ripple is already captured in quantity (1) — if consumers had to change, they show up as more touched units.
...
2. **4.2 Kvantifikace sdílení** — share ratio per module/layer/category, platform reach histogram, **layer × source-set LOC heatmap matrix** (headline "where does the code live" figure).
...
9. **4.9 Diskuse a hrozby pro validitu** — LOC metric limitations, single-developer bias, tool bias (tokei vs cloc), generated code, intermediate source set attribution, selection bias on the ProjectPhoto retro commit, the Wear OS experiment reflects the author's ability, reverse-removal ≈ forward-addition symmetry assumption, **NS theorem mapping is an interpretation by the author and not a formal NS audit**, ripple bucket classification is judgment-based and should be reproducible by a second reader. Additional explicit choices around the architectural-unit framing: (i) **architectural-unit vs build-module distinction** — counts in this chapter are reported as architectural units (`(module, source_set)`); the build-module count is given as a supporting figure, not as a primary metric, and the rationale (BE-vs-FE counting symmetry) is stated in §4.1; (ii) **empty source sets** — source sets present in the build but with zero Kotlin files are excluded from architectural-unit counts because the metric is code-bearing surface area; this affects intermediate source sets like `nonAndroidMain` in modules that have no nonAndroid-specific code; (iii) **implicit hierarchy edges** — Snag's combined source-set hierarchy uses both the explicit dependsOn wiring in `MultiplatformModuleSetup.kt` and the implicit wiring added by Kotlin's `applyDefaultHierarchyTemplate()` (e.g., `jsMain` and `wasmJsMain` descend from `webMain` via the template's `web` group, even though that edge is not in the project's own source). The closure model encodes the combined graph; `analysis/source_set_hierarchy.py` is the single source of truth and is pinned by `analysis/tests/test_source_set_hierarchy.py` so a future hierarchy change without a matching update is caught.
...
5. ✅ **Smoke-run on `main`.** 190/190 build modules covered, 310 architectural units (`(module, source_set)` rows), 48,381 total Kotlin LOC (31,773 production). Pipeline is idempotent (byte-identical output on rerun). `./gradlew check` still passes.
