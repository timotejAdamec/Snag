...
**Phase 1 — Measurement tooling (week 1)**
1. `SharingReportTask` + `SharingReportPlugin` + tests. Reuses `ModulePathParser` — no reinvention.
2. `loc_report.sh` with pinned tokei.
3. `figures.py` skeleton with real data from the first tooling run.
4. Smoke-run on `main`, inspect outputs, iterate.
