...
- For each architectural unit (`(module, source_set)` pair) present in the sharing report, computes the transitive set of downstream units that depend on it. Combines (i) the inter-module Gradle dependency graph (api/implementation scope-aware — implementation edges terminate forward propagation, directly implementing AVT), (ii) the intra-module KMP source-set `dependsOn` hierarchy encoded in `analysis/source_set_hierarchy.py` (Snag's explicit wiring in `MultiplatformModuleSetup.kt` plus the implicit edges added by `applyDefaultHierarchyTemplate()`, e.g., `jsMain`+`wasmJsMain` under `webMain`), and (iii) target-binary reachability for single-target consumers (BE modules' `main`, Android-app modules' `main`).
...
Plan doc: [`analysis/phase-3-plan.md`](phase-3-plan.md) (on `chore/phase-2-ripple-tooling`). Implementation lands on `experiment/wearos-feasibility-spike` (HEAD `2082b8d57` after step 14).
...
- [x] One emulator screenshot (`images/wear-os-project-list.png`). Captured via seed-mode composition (`-Psnag.wear.seed=true` opt-in flag in commit `7b5a179da`) because Android Studio's Wear pairing assistant installs a minimal companion stub that does not complete the `RemoteAuthClient` 3p_auth consent handshake on the emulator pair — logged as critique observation #9 with P3 refined to "HOLDS at unit-test level; end-to-end live flow deferred to real Pixel Watch hardware." The seed composition exercises the same `AuthTokenProvider` port surface as the live flow would, so the screenshot is a faithful demonstration of the architectural claim. Screenshot staged at `~/Ctu/dp-thesis-timotej-adamec/images/wear-os-project-list.png` (commits with Phase 5 §4.4 prose on a thesis-repo branch).
...
1. ~~Complete §L.2 Wear OS experiment (its own phase); land §4.4 artefacts.~~ **Done.** §4.4 numeric/critique artefacts land via Case 5 (see §K.2 entry). Emulator screenshot captured via seed-mode composition and staged at the thesis repo; emulator `RemoteAuthClient` limitation logged as critique observation #9 with P3 refined accordingly.
