...
| **T-11** | §4.1 `tab:eval-operacionalizace` | Case 3 DVT data-fill for existing TBD row. Thesis currently has "DVT synthetic scenario (optional, experiment/dvt-synthetic)" marked TBD. Corrections: (a) branch name `experiment/dvt-synthetic` → `experiment/dvt-client-field`; (b) "optional" → shipped; (c) result cell: `0 anomalies (3a non-essential = 0, 3b upward-bleed = 0, forced caller edits = 0), 1 recurring-intrinsic unit (seedTestClient test fixture), 17 files local/intrinsic/collateral = 16/1/0, churn 98/4/0 = 102 LOC`. Tooling cite `feature_retro.py`; dataset cite `analysis/data/ripple_dvt-client-field_*.csv`. | `analysis/classifications/dvt-client-field_critique.md` headline block + `ripple_dvt-client-field_units.csv` | **Pending**. Data-fill into existing row, not a row insertion. Belongs in §K.4 bundled commit. |
| **T-12** | §4.1 `tab:eval-operacionalizace` | Case 4 platform-axis data-fill for existing TBD row. Thesis currently has "iOS-only extension synthetic scenario (optional, experiment/ios-only-extension)" marked TBD. Corrections: (a) branch name `experiment/ios-only-extension` → `experiment/ios-only-project-field`; (b) "optional" → shipped; (c) result cell: `0 anomalies (commonMain = 0, non-iOS source set = 0, cross-feature = 0), 1 recurring-intrinsic unit (:root::settings), 3 files local/intrinsic/collateral = 2/1/0, churn 23/1/0 = 24 LOC`. Tooling cite `feature_retro.py`; dataset cite `analysis/data/ripple_ios-only-project-field_*.csv`. | `analysis/classifications/ios-only-project-field_critique.md` headline block + `ripple_ios-only-project-field_units.csv` | **Pending**. Data-fill into existing row, not a row insertion. Belongs in §K.4 bundled commit. |
| **T-13** | §4.6 "Případová studie 3 — DVT na entitě Client" (primary source) | Prose synthesis of the Case 3 critique file (zero-count case). Two DVT mechanisms confirmed: (1) inheritance carries shared `ico` field through `Client → AppClient → BackendClient` without redeclaration at intermediate interfaces; (2) defaults absorb at all existing construction sites with zero forced caller edits. BE-only `adminNote` has zero upward bleed into business/model, app/model, contract, or FE layers. Derivative SoC observation: be/app/model → app/model → business/model asymmetry is load-bearing for upward containment (mirror of the Case 4 downward-containment finding). | `analysis/classifications/dvt-client-field_critique.md` (shipped with Case 3) | **Pending**. Phase 5 prose authoring; critique file is the definitive source. |
| **T-14** | §4.6 "Případová studie 4 — iOS-only rozšíření entity Project" (primary source) | Prose synthesis of the Case 4 critique file (zero-count case). Platform-specific attribute `widgetPinned: Boolean` contained entirely at `iosMain` source set of the new FE-specific `feat/projects/fe/app/model/` module (mirroring the existing `be/app/model/` pattern). Zero forced `commonMain` touches, zero non-iOS-source-set touches, zero cross-feature touches. Convention-plugin auto-wire fallthrough handled the `AppProject` dependency with no explicit declaration. `archCheck` passed. | `analysis/classifications/ios-only-project-field_critique.md` (shipped with Case 4) | **Pending**. Phase 5 prose authoring; critique file is the definitive source. |
| **T-15** | §4.7 cross-case roll-up table | Expand T-8 row set (Cases 1 + 2) to all five shipped cases. Per-case row data (`recurring_intrinsic_units / collateral_files / local_files / churn_total`): **Case 1** (reverse-removal) `6 (4 distinct sites) / 43 / 89 / 6071`; **Case 1b** paired-branch comparison `correct 0/0/8 vs commonized 0/0/12`, with numeric delta `B2/B1 = 2.0×` for `blast_radius_module` and `3×` for FE leaves rebuilt; **Case 2** (forward retrospective) `1 / 4 / 90 / 4428`; **Case 3** (DVT synthetic) `1 / 0 / 16 / 102`; **Case 4** (platform-axis synthetic) `1 / 0 / 2 / 24`. | Five CSVs: `ripple_inspections-reverse-removal_units.csv`, `ripple_photo-progress-{correct,commonized}_units.csv`, `ripple_projectphoto-forward_units.csv`, `ripple_dvt-client-field_units.csv`, `ripple_ios-only-project-field_units.csv` (+ `counterfactual_photo_progress.md` for Case 1b ratios) | **Pending**. Phase 5 writes the table after all cases land. |
