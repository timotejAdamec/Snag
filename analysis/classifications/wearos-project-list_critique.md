...
**P3 — `AuthTokenProvider` substituted via port substitution alone.** **HOLDS at source/unit-test level; end-to-end live flow not demonstrable on emulator pair.** `AuthTokenProvider` interface (`feat/authentication/fe/ports/.../AuthTokenProvider.kt`) was **not edited**. `RealWearAuthFlow` + `WearAuthTokenProvider` implement the existing port contract; `OidcExecutor` (the name P3 used) does not exist in the codebase — the actual port is `AuthTokenProvider`, and substitution at that port boundary worked unchanged at compile + unit-test levels. **AVT confirmed for the auth port.** Independent end-to-end runtime confirmation deferred: Android-emulator-pair `RemoteAuthClient` could not reach consent-completion during the Phase 3 screenshot capture — see observation #9. The limitation is in the Google emulator tooling (companion-stub 3p_auth layer), not in the Snag port or adapter code; demonstrated by the Wear screen rendering correctly via the seed-mode composition using the same `AuthTokenProvider` binding surface.
...
9. **Emulator-pair `RemoteAuthClient` consent completion unreachable** (Phase 3 Step 9). Bluetooth-only OS-level pairing returned `errorCode=1` (ERROR_UNSUPPORTED) at `feat/authentication/fe/wear/driven/.../RealWearAuthFlow.kt:117-125`. Even after Android Studio's Wear pairing assistant completed a full companion pairing, the flow advanced to the Google `wear.googleapis.com/3p_auth` consent page on the phone emulator but the ALLOW button's JS callback did not complete the handshake (visible ripple on tap; no subsequent log line; no `onAuthorizationResponse` delivered to the watch). Root cause: Android Studio's assistant installs a minimal companion stub, not the full Google Play Services 3p_auth completion layer a real Pixel Watch app or Galaxy Wearable provides. **Not an observation about Snag's architecture** — the port substitution + adapter code work correctly at compile + unit-test level — but a concrete cost of the emulator-only QA path. Workaround: `:wearApp` gained an opt-in `BuildConfig.SEED_MODE` flag (gated on `-Psnag.wear.seed=true`; commit `7b5a179da` on `experiment/wearos-feasibility-spike`) that swaps `wearAppModule` for a `wearSeedModule` binding fake `AuthTokenProvider` + fake use cases, letting the Wear UI composition render end-to-end for the §4.4 demonstration screenshot. The seed bindings use the *same* `AuthTokenProvider` port interface — so the screenshot exercises the same architectural surface as the live flow would. **NS-theorem: AVT** (graded — further reinforces the port substitutability claim, because the seed test-double slots in at the same port without any code outside `:wearApp` noticing). Cross-references §4.9 future work: "full Wear end-to-end OIDC demonstration deferred to when real Pixel Watch hardware is available; adapter verified at unit-test level + end-to-end architectural integration verified via seed-mode screenshot."
