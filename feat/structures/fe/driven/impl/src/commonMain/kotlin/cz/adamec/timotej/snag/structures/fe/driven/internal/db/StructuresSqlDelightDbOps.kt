...
internal class StructuresSqlDelightDbOps(
...
) : SqlDelightDbOps<cz.adamec.timotej.snag.feat.shared.database.fe.db.StructureEntity, FrontendStructure>(
    ...
    fun structuresByProjectIdFlow(projectId: Uuid) =
        entitiesByQueryFlow(queries.selectByProjectId(projectId.toString()))

    suspend fun getStructureIdsByProjectId(projectId: Uuid): List<Uuid> =
        withContext(ioDispatcher) {
            queries.selectIdsByProjectId(projectId.toString())
                .executeAsList()
                .map { Uuid.parse(it) }
        }
