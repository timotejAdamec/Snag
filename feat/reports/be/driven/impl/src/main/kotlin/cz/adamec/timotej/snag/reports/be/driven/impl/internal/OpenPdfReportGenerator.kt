...
internal class OpenPdfReportGenerator : PdfReportGenerator {
    ...
    private fun addInspectionsSection(
        ...
        data.inspections.forEach { inspection ->
            table.addCell(createCell(inspection.startedAt?.let { formatTimestamp(it.value) } ?: "-"))
            table.addCell(createCell(inspection.endedAt?.let { formatTimestamp(it.value) } ?: "-"))
            table.addCell(createCell(inspection.participants ?: "-"))
            table.addCell(createCell(inspection.climate ?: "-"))
            table.addCell(createCell(inspection.note ?: "-"))
        }
    ...
    private fun addFloorPlan(
        ...
        val imageBytesResult =
            runCatching { URI(floorPlanUrl).toURL().readBytes() }
                .onFailure {
                    ...
                    LH.logger.warn("Floor plan image could not be loaded: $floorPlanUrl. Exception: $it")
                }
        ...
        val imageResult =
            runCatching { Image.getInstance(annotatedBytes) }
                .onFailure {
                    document.add(Paragraph("Floor plan image could not be loaded.", FONT_SMALL))
                    LH.logger.warn("Floor plan image could not be rendered: $floorPlanUrl. Exception: $it")
                }
        val image = imageResult.getOrNull() ?: return
