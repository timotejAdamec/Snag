/*
 * Copyright (c) 2026 Timotej Adamec
 * SPDX-License-Identifier: MIT
 *
 * This file is part of the thesis:
 * "Multiplatform snagging system with code sharing maximisation"
 *
 * Czech Technical University in Prague
 * Faculty of Information Technology
 * Department of Software Engineering
 */

package cz.adamec.timotej.snag.feat.inspections.fe.driven.test

import cz.adamec.timotej.snag.feat.inspections.fe.ports.InspectionsSync
import kotlin.uuid.Uuid

class FakeInspectionsSync : InspectionsSync {
    val savedInspectionIds = mutableListOf<Uuid>()
    val deletedInspectionIds = mutableListOf<Uuid>()

    override suspend fun enqueueInspectionSave(inspectionId: Uuid) {
        savedInspectionIds.add(inspectionId)
    }

    override suspend fun enqueueInspectionDelete(inspectionId: Uuid) {
        deletedInspectionIds.add(inspectionId)
    }
}
