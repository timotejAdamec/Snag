...
internal class GetProjectAssignmentsUseCaseImpl(
    ...
    override suspend operator fun invoke(projectId: Uuid): List<BackendUser> {
        logger.debug("Getting assigned users for project {}.", projectId)
        return projectAssignmentsDb.getAssignedUsers(projectId).also {
            logger.debug("Got assigned users for project {}.", projectId)
        }
    }
}
