...
class AssignUserToProjectUseCaseImplTest : FrontendKoinInitializedTest() {
    ...
    @Test
    fun `assigns user via API and refreshes local DB`() =
    ...
    @Test
    fun `local DB is refreshed from API after assign`() =
        runTest(testDispatcher) {
            val existingUserId = UuidProvider.getUuid()
            fakeProjectsApi.projectAssignments[projectId] = setOf(existingUserId, userId)
            ...
            val result = fakeProjectAssignmentsDb.getAssignedUserIdsFlow(projectId).first()
            assertIs<OfflineFirstDataResult.Success<Set<*>>>(result)
            assertEquals(setOf(existingUserId, userId), result.data)
        }
