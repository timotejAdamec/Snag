...
| FP35 | Deny access without authentication | — | - [x] BE: unauthenticated requests get no `CurrentUser` → `currentUser()` throws → 401. FE: `AuthenticationGate` shows `LoginScreen` until `GetAuthenticatedUserIdUseCase` emits non-null user ID. |
