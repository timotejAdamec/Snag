...
3. ~~**EntraID authentication**~~ — **Done (production wiring pending).** BE: dual-mode `CallCurrentUserPlugin` validates EntraID JWT via `EntraIdJwtVerifier` in production, retains mock header for dev. Auto-creates user on first login. FE: `AuthTokenProvider` with mock/MSAL implementations, login screen gate, `CurrentUserIdStore`. Build-time config via BuildKonfig (`MOCK_AUTH`, `ENTRA_ID_TENANT_ID`, `ENTRA_ID_CLIENT_ID`). See `docs/entra_id_setup.md` for production deployment steps. *(§4, FP34, FP35, NP12)*
   - **Remaining for production deployment**: (a) register app in Azure Portal and fill in tenant/client IDs in `config/release.properties`; (b) add `auth_config.json` to Android `res/raw/`; (c) wire `MsalAuthTokenProvider` into Koin DI for Android when `mockAuth=false`; (d) resolve `CurrentUserIdStore` from MSAL token after login + first sync; (e) connect `LoginScreen.onSignIn` to MSAL interactive flow. These are deployment/configuration tasks — the architecture and all code paths are implemented.
