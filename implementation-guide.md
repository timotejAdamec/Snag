...
- [x] Login exclusively via **corporate Microsoft EntraID**. *(BE: `CurrentUserConfiguration` dual-mode — validates EntraID JWT via Ktor `Authentication` plugin with `jwt {}` provider in production, retains `X-User-Id` header mock mode for dev/testing via `RunConfig.mockAuth`. FE: hexagonal architecture — `AuthTokenProvider` port with `OidcAuthTokenProvider` (kotlin-multiplatform-oidc v0.16.5, Auth Code + PKCE, token refresh built into provider, persistent secure storage) and `MockAuthTokenProvider` (hardcoded UUID). `LoginUseCase` coordinates OIDC login → `GET /users/me` → set authenticated user ID. FE bearer auth via Ktor `bearer {}` provider in `CurrentUserHttpClientConfiguration` (production) or mock header plugin (dev). `AuthenticationGate` composable wraps `MainScreen` as a precondition gate. Build-time config: `MOCK_AUTH`, `ENTRA_ID_TENANT_ID`, `ENTRA_ID_CLIENT_ID`, `ENTRA_ID_MOBILE_REDIRECT_URI`, `ENTRA_ID_JVM_REDIRECT_URI`, `ENTRA_ID_WEB_REDIRECT_PATH` in BuildKonfig.)*
- [x] No invite flow, no password management in the system.
- [x] A user without a valid EntraID token has no access whatsoever. *(BE: `CurrentUserConfiguration` only sets `CurrentUser` attribute on valid JWT or valid mock header; routes call `currentUser()` which throws `UnauthenticatedException` → 401 when attribute is missing. FE: `AuthenticationGate` with `AuthenticationViewModel` observes `IsCurrentUserAuthenticatedFlowUseCase` — `LoginScreen` shown until authenticated.)*
- [x] After successful authentication the system maps the EntraID identity to a role stored in the system. *(BE: `CurrentUserConfiguration` extracts `oid` claim from `JWTPrincipal`, resolves user via `GetOrCreateUserByAuthProviderIdUseCase`. Auto-creates user with `role=null` on first login. FE: `LoginUseCase` calls `AuthTokenProvider.login()` for OIDC flow, then calls `GET /users/me` to resolve user ID, then sets `AuthState.Authenticated(authProviderId)`. User's role is stored in DB and synced to FE.)*
...
2. - [x] **User assignment to / removal from project** — leads can add/remove technicians or service workers. *(BE: API + DB + role-restricted access enforced via `CanAssignUserToProjectRule` — Admin/VP/VS allowed; TP/Ser/None denied. FE: `AssignUserToProjectUseCase` + `RemoveUserFromProjectUseCase` implemented and wired into `ProjectDetailsViewModel`; local assignment cache with full-replacement pull sync, cascade delete/restore on project deletion, query use case.)*
3. - [x] **Authentication (EntraID)** — mandatory Microsoft EntraID login (standalone mechanism, not a UC). *(BE: dual-mode `CurrentUserConfiguration` with JWT validation via `EntraIdJwtVerifier` + mock header fallback; `GetOrCreateUserByAuthProviderIdUseCase` auto-creates users; `GET /users/me` endpoint. FE: hexagonal `feat/authentication/fe/` — `AuthTokenProvider` port with `OidcAuthTokenProvider` (kotlin-multiplatform-oidc) / `MockAuthTokenProvider`, `LoginUseCase` (OIDC → /users/me → auth provider ID), `AuthenticationGate` precondition gate. Build-time config via BuildKonfig.)*
...
| FP4c | Assign user to project | UC1 | - [x] BE API + DB + role-restricted access enforced via `CanAssignUserToProjectRule`. FE: `AssignUserToProjectUseCase` implemented; local assignment cache synced from BE (full-replacement pull sync). |
| FP4d | Remove user from project | UC1 | - [x] BE API + DB + role-restricted access enforced via `CanAssignUserToProjectRule`. FE: `RemoveUserFromProjectUseCase` implemented; cascade delete on project deletion. |
...
- [x] **Backend**: validate EntraID JWT on every protected endpoint; reject requests without valid token. *(Dual-mode `CurrentUserConfiguration`: production validates JWT via Ktor `Authentication` plugin with `jwt {}` provider against EntraID JWKS; dev uses `X-User-Id` header via `RunConfig.mockAuth`. `GetOrCreateUserByAuthProviderIdUseCase` auto-creates user on first login. `GET /users/me` returns current user's `UserApiDto`.)*
- [x] **Frontend**: implement OAuth2 / OIDC flow with EntraID; store tokens securely; refresh tokens before expiry. *(Hexagonal architecture in `feat/authentication/fe/`: use cases in `app/{api,impl}` (`IsCurrentUserAuthenticatedFlowUseCase`, `GetAuthProviderIdUseCase`, `GetAccessTokenUseCase`, `LoginUseCase` coordinates OIDC login + `/users/me` user resolution, `LogoutUseCase`, `RestoreSessionUseCase`); port interfaces in `ports` (`AuthTokenProvider` with `AuthState` sealed — `Loading`/`Unauthenticated`/`Authenticated(authProviderId: String)`); adapters in `driven/impl` (`OidcAuthTokenProvider` via kotlin-multiplatform-oidc v0.16.5 — Auth Code + PKCE, persistent secure storage, token refresh built into provider; `MockAuthTokenProvider` with hardcoded UUID); FE bearer auth via Ktor `bearer {}` provider in `CurrentUserHttpClientConfiguration`; test fakes in `driven/test` (`FakeAuthTokenProvider`); UI in `driving/impl` (`AuthenticationGate` precondition composable wrapping `MainScreen`, `LoginScreen` with loading state, `AuthenticationViewModel`). Mock/production switching via `RunConfig.mockAuth` DI.)*
...
- [x] **Frontend**: FE authorization use cases wrap shared KMP rules with `GetCurrentUserFlowUseCase` and reactive DB queries, emitting `Flow<Boolean>` (deny-first: all default to `false`). ViewModels collect these into state booleans that gate UI elements.
  ...
  - Current user is resolved via `GetAuthProviderIdUseCase` (from `feat/authentication/fe/app/api`), which maps `AuthState` to the auth provider ID. `GetCurrentUserFlowUseCaseImpl` uses the resulting ID to query user data (including role) from local DB. In mock mode, `MockAuthTokenProvider` provides a hardcoded UUID eagerly. In production, `LoginUseCase` coordinates OIDC login → `/users/me` call → `AuthState.Authenticated(authProviderId)` resolution.
